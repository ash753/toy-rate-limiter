...
## Step 8. nGrinder 테스트 / 정책 검증

**목표:** 이동 윈도우 가중 평균 알고리즘의 정확도를 검증하고, Kubernetes 환경의 다중 인스턴스에서 전역 카운팅이 정확한지 nGrinder를 통해 시뮬레이션한다.
...
- [ ] 다중 인스턴스 테스트 (Kubernetes)
  - `kubectl scale deployment rate-limiter --replicas=3` 등 스케일링 후 테스트
